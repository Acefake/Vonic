...
export const windowAPI: WindowAPI = {
  ...
  current: createWindowInstance(null),

  // 动态生成所有窗口的 API（自动包含 WindowName 枚举中的所有窗口）
  ...Object.values(WindowNameEnum).reduce((acc, windowName) => {
    // 过滤出字符串值（TypeScript 枚举会包含键和值）
    if (typeof windowName === 'string') {
      acc[windowName as WindowName] = createWindowInstance(windowName as WindowName)
    }
    return acc
  }, {} as Record<WindowName, WindowInstanceAPI>),
} as WindowAPI
