...
import { createWebWindowAPI } from '../platform/window.web'
...
class Window implements WindowInstanceAPI {
  ...
  async open(options?: WindowCreateOptions & { waitForResult?: boolean }, waitForResult?: boolean): Promise<unknown> {
    ...
    if (shouldWaitForResult) {
      ...
      return ipc.invoke('window:create-async', this.windowName, restOptions)
    }
    // 如果明确指定不等待结果，使用 send 方式（立即返回）
    ipc.send('window:create', this.windowName, options)
  ...
  close(result?: unknown, windowId?: number): void {
    ...
    if (this.windowName === null) {
      // 当前窗口实例：关闭当前窗口
      ipc.send('window:close', serializedResult)
    }
    else {
      // 特定窗口实例：关闭指定窗口或所有同名窗口
      ipc.send('window:close', serializedResult, windowId, this.windowName)
    }
  }
  ...
  async getAll(): Promise<WindowInstanceInfo[]> {
    ...
    return ipc.invoke('window:get-all', this.windowName) as Promise<WindowInstanceInfo[]>
  }
  ...
  async getById(windowId: number): Promise<WindowInstanceInfo | null> {
    ...
    return ipc.invoke('window:get-by-id', this.windowName, windowId) as Promise<WindowInstanceInfo | null>
  }
  ...
  async getConfig(): Promise<WindowConfig | null> {
    const result = await ipc.invoke('window:get-config')
  ...
  async getData<T = unknown>(): Promise<T | null> {
    const result = await ipc.invoke('window:get-data')
  ...
  updateData(data: unknown): void {
    ...
    ipc.send('window:update-data', serializedData)
  }
}
 ...
 */
export const windowAPI: WindowAPI = isElectron
  ? new Proxy({} as WindowAPI, windowAPIProxyHandler)
  : createWebWindowAPI()
